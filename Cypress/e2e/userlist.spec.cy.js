describe('User List Page', () => {
  beforeEach(() => {
    cy.visit('/userList') // Visit the user list page before each test
  })
  ...
  it('should navigate to the user list page when the Users button is clicked', () => {
    cy.contains('Hotels').click({ force: true }) // Click on the Users button
    cy.url().should('include', '/hotelList') })

  it('should navigate to the rider List when the Riders button is clicked', () => {
    cy.contains('Riders').click({ force: true }) 
    cy.url().should('include', '/riderList') 
  })
